...
struct RegionGeneratorFixture {
    RegionGeneratorFixture() {
        CgnsReader cgnsReader(this->inputPath);
        this->gridData = cgnsReader.gridData;
    }

    std::string inputPath = std::string(TEST_INPUT_DIRECTORY) + "Manipulation/RegionGenerator/125v_64e.cgns";
    boost::shared_ptr<GridData> gridData;
    std::string regionGeneratorScript = std::string(TEST_INPUT_DIRECTORY) + "Manipulation/RegionGenerator/RegionGenerator.json";
};

FixtureTestSuite(RegionGeneratorSuite, RegionGeneratorFixture)
